...
## D21 — Live endpoint verification closes the defined integration gate
**Status: CLOSED 2026-09-12 — Four UTC-stamped direct-endpoint checks passed while `origin/main` matched expected source commit `f258473`; exact Railway runtime SHA was not exposed.**

The live `/chat` request exercised sanitized quiz context, Claude, Shopify catalog search, two product cards, exact `create_cart` invocation, assistant cart confirmation, and a native checkout handoff. The SSE contract does not expose raw Shopify tool results, so this establishes invocation and handoff rather than an independently persisted cart-result event. The `/vision-copy` upload endpoint returned structured Claude copy. `/generate-image` returned a validated OpenAI PNG and a session-bound reference that `/vision-copy` accepted for a second structured Claude response. A deterministic image containing `ALEX` and `BIRTHDAY CREW` returned none of those terms and exactly matched every field of the fixed NR-8 fallback.
